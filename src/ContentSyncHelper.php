...
class ContentSyncHelper {
     ...
     * @return NULL[]
     */
    public static function exportEntity(EntityInterface $entity, array $skiped_fields = [], array &$docs = []) {
        if (empty($entity) || !empty($docs[$entity->uuid()])) return $docs;
        $serializer = \Drupal::service('serializer');
        $docs[$entity->uuid()] = $serializer->normalize($entity, 'json', ['new_revision_id' => FALSE]);
                    ...
                    foreach ($field as $index => $item) {
                        if($fieldType == 'panelizer') {
                                ...
                                if ($blocks) {
                                    foreach ($blocks as $block_id => $block){
                                        if ($block->getBaseId() == 'block_content') {
                                            ...
                                            if($block_content){
                                                static::exportEntity($block_content, $skiped_fields, $docs);
                                            }
                                        }
                                    }
                                }
                                ...
                                foreach ($xpath->query('//*[@data-entity-type and @data-entity-uuid]') as $node) {
                                    ...
                                    try {
                                        ...
                                        if(!empty($embed_entity)){
                                            ...
                                            static::exportEntity($embed_entity, $skiped_fields, $docs);
                                        }
                                    } catch (\Exception $e) {
                                    }
                                }
                            ...
                            if (!empty($item->entity)) {
                                static::exportEntity($item->entity, $skiped_fields, $docs);
                            }
                        }
                    }
                }
