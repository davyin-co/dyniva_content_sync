...
namespace Drupal\dyniva_content_sync\BulkDocs;
...
class BulkDocs implements BulkDocsInterface {
  ...
  public function save() {
    ...
    do {
      ...
      if ($this->lock->lockMayBeAvailable('dyniva_content_sync_bulk_docs')) {
        // The operation may be available, so break the wait and continue if we
        // successfully can acquire a lock.
        if ($this->lock->acquire('dyniva_content_sync_bulk_docs')) {
          break;
        }
      }
      $this->logger->critical('Lock exists on bulk operation. Waiting.');
    } while ($this->lock->wait('dyniva_content_sync_bulk_docs', 3000));
    ...
    foreach ($this->entities as $entity) {
      ...
      try {
        ...
        if(\Drupal::service('multiversion.manager')->isEnabledEntityType($entity->getEntityType())) {
          ...
          if ($record) {
            if (!$this->newEdits && !$record['is_stub'] && $record['entity_id'] != 0 && $record['revision_id'] != 0) {
              continue;
            }
          }
          ...
          if ($record = $this->uuidIndex->useWorkspace($this->workspace->id())->get($uuid)) {
...
//           if ($record = \Drupal::entityManager()->loadEntityByUuid($entity->getEntityTypeId(), $uuid)) {
//             continue;
//           }
          if ($entity->save()) {
            $this->result[] = [
    ...
    $this->lock->release('dyniva_content_sync_bulk_docs');
    return $this;
  }
